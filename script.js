...
// Phases Data
const phases = {
    1: { name: "Hypertrophy", label: "Hypertrophy", weeks: "Weeks 1-4", focus: "Tissue Capacity", reps: "3 x 10-12",
        workouts: {
    ...
    2: { name: "Strength", label: "Strength", weeks: "Weeks 5-8", focus: "Force Production", reps: "5 x 3-5",
        workouts: {
            Monday: { title: "Contrast Squat", focus: "Max Force", exercises: [
                { name: "Back Squat", sets: "4x4 @ 85%", completed: false },
                { name: "Vertical Jump", sets: "4x3", completed: false },
            ...
            Wednesday: { title: "Recovery", focus: "Mobility", exercises: [{ name: "Pool Work", sets: "20 mins", completed: false }, { name: "Hip Flow", sets: "15 mins", completed: false }] },
            ...
            Saturday: { title: "Dunk Session", focus: "Max Touch", exercises: [{ name: "Lob Dunks", sets: "10-15 reps", completed: false }] },
            Sunday: { title: "Rest", focus: "Sleep", exercises: [{ name: "Sleep", sets: "10 hrs", completed: false }] }
        }},
    3: { name: "Power", label: "Power", weeks: "Weeks 9-12", focus: "Velocity", reps: "4 x 3",
        workouts: {
            ...
            Saturday: { title: "Dunk Day", focus: "PR Attempt", exercises: [{ name: "Dunk Attempts", sets: "Max Effort", completed: false }] },
            Sunday: { title: "Rest", focus: "Sleep", exercises: [{ name: "Sleep", sets: "10 hrs", completed: false }] }
        }}
};
...
const app = {
    init: () => {
        ...
        // Load correct dropdown options for initial hydration
            ...
            if (state.currentPhase === 'recovery') {
                ...
                exercisesHTML = workout.exercises.map((ex, idx) => {
                    ...
                    <div onclick="app.actions.toggleExercise('${state.selectedDay}', ${idx})" class="group p-4 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${ex.completed ? 'bg-green-900/10 border-green-900/30' : 'bg-neutral-800/50 border-neutral-800 hover:border-neutral-600'}">
                        <div>
                            <div class="font-bold text-sm ${ex.completed ? 'text-green-500 line-through decoration-green-500/50' : 'text-white'} flex items-center">${finalName} ${isSwapped ? `<span class="ml-2 text-[8px] bg-red-900/50 text-red-200 px-1.5 py-0.5 rounded uppercase tracking-wider font-black">SWAP: ${swapReason}</span>` : ''}</div>
                            <div class="text-xs text-neutral-500 mt-1 font-mono">${phaseData.reps}</div>
                        </div>
                        <div class="w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${ex.completed ? 'border-green-500 bg-green-500' : 'border-neutral-700 group-hover:border-neutral-500'}">${ex.completed ? '<i data-lucide="check" class="w-3.5 h-3.5 text-black stroke-[3px]"></i>' : ''}</div>
                    </div>`;
                }).join('');
            }
